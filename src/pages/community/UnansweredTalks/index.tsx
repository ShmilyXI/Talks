...
const TopContributors = () => {
  ...
  const [unansweredTalkList, setUnansweredTalkList] = useState<UnansweredTalkItem[]>(); // 文章列表
  ...
  return (
    <div>
      ...
      <p className="text-14 leading-md mb-16">没有评论的讨论。成为第一个发表评论的人。</p>
        ...
        {unansweredTalkList?.length
          ? unansweredTalkList.map((item) => (
              <li className="py-12" key={item.articleTitle}>
                <div className="my-8">
                  <a href={item.articleLink} className="text-black block hover:text-black font-medium">
                    {item.articleTitle}
                  </a>
                </div>
                ...
                <div className="flex items-center my-8">
                  ...
                  <div className="text-14 truncate">
                    <a href={item.authorLink} target="_blank" className="text-black mr-2 hover:text-black">
                      {item.author}
                    </a>
                  </div>
                  <div className="flex items-center text-12">
                    <Icon className="icon-comment" addClassName="text-16 mr-1 text-black" />
                    <a href={`${item.articleLink}#comments`} className="text-grey-53 hover:text-grey-53">
                      {item.answerCount}
                    </a>
                  </div>
                </div>
