...
export function showErrorMiddleware(option: ShowErrorConfig): Middleware<Context> {
  const { showError, handleError, enable = true } = option;
  return async (ctx) => {
    const { customOptions: { errorEnabled: isShowError = enable, showError: requestShowError } = {} } = ctx.config as ShowErrorOption;
