...
const Mood: FC<MoodProps> = (props) => {
  ...
  const [
    showMoodModal,
    { toggle: toggleMoodModal, setLeft: setMoodModalLeft },
  ] = useToggle(); // 是否展示表情选择弹窗

  // 获取标签元素列表
  const getMoonMenuItems = (): IItem[] => {
    const items: IItem[] = Object.keys(MoodEnum).reduce(
      (pre: IItem[], cur: string) => {
        pre.push({
          label: (
            <button
              type="button"
              className="button-reset flex items-center w-full text-left py-8 lg:py-1 hover:no-underline"
            >
              <span className="text-18 leading-none mr-8">{MoodEnum[cur]}</span>
  ...
  return (
    <div>
      <Menu
        ...
        value={value}
        items={!disabled ? getMoonMenuItems() : []}
        setLeft={setMoodModalLeft}
        onChange={(item) => {
      ...
      >
        ...
        >
          <span className="off" title="Select mood">
            <span className="button--pill__icon">
              <svg
                className="icon"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 496 512"
          ...
          <span className="on">
            <span className="button--pill__icon mr-6">{MoodEnum?.[value]}</span>
